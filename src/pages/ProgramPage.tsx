...
export default function ProgramPage() {
  ...
  const maxIndex = roadmapCards.length - 2;
    ...
    />
      ...
      <section className="relative overflow-hidden bg-[#0b0b0b]" style={{ backgroundImage: "linear-gradient(rgba(7,180,186,.07) 1px,transparent .4px),linear-gradient(90deg,rgba(7,180,186,.07) 1px,transparent .4px)", backgroundSize: "30px 30px" }}>
        <div className="w-full py-10 md:py-12" style={GUTTER}>
          ...
          {/* Mobile: horizontal row cards stacked */}
          <div className="flex flex-col gap-3 md:hidden">
            {whatCards.map((item, i) => (
              <Reveal key={i} delay={i * 60}>
                <div className="w-full flex flex-row items-center gap-4 p-4 rounded-[14px] bg-gradient-to-b from-[#13171d] to-[#101318] border border-white/5">
                  <div className="w-[48px] h-[48px] flex items-center justify-center shrink-0">{item.icon}</div>
                  <div className="flex flex-col flex-1 min-w-0">
                    <h4 className="font-['Bebas_Neue'] text-[#07b4ba] text-[15px] tracking-[1.5px] leading-[1.3] mb-[3px]">{item.title}</h4>
                    <p className="text-[12px] leading-[1.5] text-white/60 m-0">{item.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
          ...
          {/* Desktop: grid */}
          <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-5 gap-[16px]">
            {whatCards.map((item, i) => (
              <Reveal key={i} delay={i * 80}>
                <div className="w-full p-[16px] rounded-[18px] bg-gradient-to-b from-[#13171d] to-[#101318] border border-white/5 flex flex-col items-center text-center gap-3 min-h-[255px]">
                  <div className="w-[70px] h-[70px] flex items-center justify-center shrink-0">{item.icon}</div>
                  <div className="flex flex-col items-center w-full">
                    <h4 className="font-['Bebas_Neue'] text-[#07b4ba] text-[17.5px] tracking-[2px] leading-[1.3] m-0 text-center mb-[3px]">{item.title}</h4>
                    <p className="text-[14px] leading-[1.5] text-white/60 text-center m-0">{item.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
      ...
      {/* ── ROADMAP SECTION — unified layout for all screen sizes ── */}
      <div className="relative overflow-hidden bg-[#0b0b0b]">
        <div className="w-full py-8" style={{ backgroundImage: "repeating-linear-gradient(-45deg,rgba(7,180,186,.04) 0px,rgba(7,180,186,.04) 1px,transparent 1px,transparent 6px)" }}>
          ...
          {/* ── Unified Roadmap: same desktop-style layout on all screens ── */}
          <div className="relative" style={{ paddingLeft: "calc(1cm + 18px)", paddingRight: "calc(1cm + 18px)" }}>
            {/* Prev arrow */}
            <button
              disabled={roadmapIndex === 0}
              onClick={() => setRoadmapIndex(p => Math.max(p - 1, 0))}
              className="absolute left-0 top-1/2 -translate-y-1/2 z-20 w-[36px] h-[36px] md:w-[52px] md:h-[52px] rounded-[10px] md:rounded-[14px] border border-white/10 bg-[#0d1117] text-white text-xl md:text-2xl cursor-pointer flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed transition-opacity"
            >‹</button>
            {/* Next arrow */}
            <button
              disabled={roadmapIndex >= maxIndex}
              onClick={() => setRoadmapIndex(p => Math.min(p + 1, maxIndex))}
              className="absolute right-0 top-1/2 -translate-y-1/2 z-20 w-[36px] h-[36px] md:w-[52px] md:h-[52px] rounded-[10px] md:rounded-[14px] border border-white/10 bg-[#0d1117] text-white text-xl md:text-2xl cursor-pointer flex items-center justify-center disabled:opacity-30 disabled:cursor-not-allowed transition-opacity"
            >›</button>
            ...
            <div className="flex justify-between mb-6 md:mb-10 relative">
              ...
              {roadmapCards.map((week, i) => (
                <div key={i} className="relative z-10 text-center flex-1">
                  <p className={`font-['Bebas_Neue'] text-[9px] md:text-[15px] tracking-[0.5px] md:tracking-[1px] mb-2 md:mb-2.5 transition-colors leading-tight px-0.5 ${i === roadmapIndex || i === roadmapIndex + 1 ? "text-[#07b4ba]" : "text-white/45"}`}>{week.title}</p>
                  <div className={`w-[16px] h-[16px] md:w-[26px] md:h-[26px] mx-auto rounded-full border-2 border-[#07b4ba] transition-all ${i === roadmapIndex || i === roadmapIndex + 1 ? "bg-[#07b4ba] shadow-[0_0_12px_rgba(7,180,186,.95)]" : "bg-[#0b0b0b]"}`} />
                </div>
              ))}
            </div>
            ...
            <div className="overflow-hidden">
              <div
                className="flex gap-3 md:gap-5 transition-transform duration-[450ms] ease-in-out"
                style={{ transform: `translateX(-${roadmapIndex * 47}%)` }}
              >
                {roadmapCards.map((card, i) => (
                  <div key={i} className="min-w-[92vw] md:min-w-[45%] rounded-[14px] md:rounded-[22px] overflow-hidden bg-gradient-to-b from-[#10151d] to-[#0b0f14] border border-white/5 flex-shrink-0">
                    <div className="grid grid-cols-2">
                      <img src={card.image} alt={card.title} className="w-full h-[160px] md:h-[285px] object-cover" />
                      <div className="p-4 md:p-8 flex flex-col justify-center">
                        <h3 className="font-['Bebas_Neue'] text-[20px] md:text-[42px] text-white mb-2 md:mb-4 leading-none">{card.title}</h3>
                        ...
                        <div className="flex flex-col gap-2 md:gap-3">
                          {card.points.map((pt, pi) => (
                            <div key={pi} className="flex items-center gap-1.5 md:gap-2.5">
                              <div className="w-4 h-4 md:w-5 md:h-5 rounded-full border-2 border-[#07b4ba] text-[#07b4ba] flex items-center justify-center text-[8px] md:text-[10px] shrink-0">✓</div>
                              <p className="text-[11px] md:text-[14px] text-white/75 leading-snug">{pt}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                    </div>
                    <div className="py-3 md:py-4 border-t border-white/5 text-center">
                      <p className="font-['Bebas_Neue'] text-[16px] md:text-[22px] text-[#07b4ba] tracking-[1px]">{card.days}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="flex justify-center gap-2 mt-5">
              {roadmapCards.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setRoadmapIndex(Math.min(i, maxIndex))}
                  className={`w-2 h-2 p-0 border-0 rounded-full cursor-pointer transition-all ${(i === roadmapIndex || i === roadmapIndex + 1) ? "bg-[#07b4ba] shadow-[0_0_8px_rgba(7,180,186,.7)]" : "bg-white/30"}`}
                />
              ))}
            </div>
          </div>
      ...
      <div className="relative overflow-hidden bg-[#0b0b0b]" style={{ backgroundImage: "linear-gradient(rgba(7,180,186,.05) 1px,transparent .4px),linear-gradient(90deg,rgba(7,180,186,.05) 1px,transparent .4px)", backgroundSize: "32px 32px" }}>
        <div className="w-full py-10 md:py-12" style={GUTTER}>
          ...
          {/* Mobile: horizontal row bonus cards stacked */}
          <div className="flex flex-col gap-3 md:hidden">
            {[
              ...
              <Reveal key={i} delay={i * 60}>
                <div className="w-full flex flex-row items-center gap-4 p-4 rounded-[14px] bg-gradient-to-b from-[#0f1115] to-[#0a0f14] border border-[#07b4ba]/30 relative overflow-hidden">
                  <div className="absolute top-2 left-2 bg-[#07b4ba] text-[#111] font-['Bebas_Neue'] text-[11px] tracking-[1px] px-1.5 py-0.5 rounded-[4px] leading-none">#{i + 1}</div>
                  <div className="w-[48px] h-[48px] flex items-center justify-center shrink-0 mt-2">{item.icon}</div>
                  <div className="flex flex-col flex-1 min-w-0 mt-1">
                    <h3 className="font-['Bebas_Neue'] text-[14px] leading-[1.1] tracking-[1px] text-white mb-1">{item.title}</h3>
                    <p className="font-['Barlow'] text-[11px] text-white/55 leading-[1.5] m-0">{item.desc}</p>
                  </div>
                </div>
              </Reveal>
            ))}
          ...
          <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-5 gap-2">
            {[
              ...
              <Reveal key={i} delay={i * 60}>
                <div className={`bg-gradient-to-b from-[#0f1115] to-[#0a0f14] border border-[#07b4ba]/30 rounded-[18px] p-4 relative overflow-hidden min-h-[220px] flex flex-col items-center text-center ${i === 4 ? "col-span-2 lg:col-span-1" : ""}`}>
                  <div className="absolute top-3 left-3 bg-[#07b4ba] text-[#111] font-['Bebas_Neue'] text-[14px] tracking-[1px] px-2 py-0.5 rounded-[5px] leading-none">#{i + 1}</div>
                  <div className="w-[70px] h-[70px] flex items-center justify-center mb-2 mt-4">{item.icon}</div>
                  <h3 className="font-['Bebas_Neue'] text-[18px] leading-[1.1] tracking-[1.5px] text-white mb-1.5">{item.title}</h3>
                  <p className="font-['Barlow'] text-[13px] text-white/55 leading-[1.55] px-1">{item.desc}</p>
                </div>
              </Reveal>
            ))}
          </div>
